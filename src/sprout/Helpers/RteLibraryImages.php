...
class RteLibraryImages extends RteLibrary
{
    ...
    public function browse($path)
    {
        ...
        if ($path == '') {
            ...
            $q = "SELECT files.name, files.filename, files.date_modified
                ...
                ORDER BY files.name";
            $res = Pdb::query($q, [$cat_id, FileConstants::TYPE_IMAGE], 'pdo');

            $size = Kohana::config('file.imagelink_size');
            if (! $size) $size = 'r500x500';
            ...
            foreach ($res as $row) {
                $out[] = new RteLibObject(
                    ...
                    $row['name'],
                    array(
                        'href' => File::resizeUrl($row['filename'], $size),
                        'title' => $row['name'],
                    ),
                    array(
    ...
    **/
    public function search($term)
    {
        ...
        $params = [FileConstants::TYPE_IMAGE];
