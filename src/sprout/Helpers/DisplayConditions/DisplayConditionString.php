 ...
 */
abstract class DisplayConditionString extends DisplayCondition
{
    ...
    public function match(array $env, $op, $val)
    {
        ...
        switch ($op) {
            ...
            case '!=':
                return ($curr_val != $val);
            case 'c':
                return (strpos($curr_val, $val) !== false);
            case 'nc':
                return (strpos($curr_val, $val) === false);
            case 'b':
                return (strpos($curr_val, $val) === 0);
            case 'e':
                return (substr($curr_val, 0 - strlen($val)) === $val);
            default:
                return false;
        }
    }
