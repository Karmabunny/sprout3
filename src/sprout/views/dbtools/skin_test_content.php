...
<form class="forms-form" method="post" action="form/submit/1">
    ...
    <?php foreach ($form_attributes as $label => $attributes): ?>
        <h2><?= Enc::html($label); ?></h2>

        <?php foreach (Fb::$methods as $method):
            Form::nextFieldDetails($method, false);
            echo Form::$method(sprintf('%s,%s', $method, $method), $attributes, $form_options);
        endforeach; ?>
